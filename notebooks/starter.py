...
def _(mo):
    mo.md(r"""
    # Oedokumaci Testing - Starter Notebook
...
@app.cell
def _(mo, slider):
    # Cells automatically re-run when UI elements change
    mo.md(f"You selected: **{slider.value}** → squared = **{slider.value ** 2}**")
    return
...
def _(mo):
    ...
    framework = mo.ui.dropdown(
        options=["pandas", "polars", "duckdb"],
        value="pandas",
        label="Favorite DataFrame library",
    )
    framework
    return (framework,)
...
def _(framework, mo):
    messages = {
        "pandas": "📊 Classic choice! Great ecosystem and documentation.",
        "polars": "⚡ Blazing fast! Rust-powered performance.",
