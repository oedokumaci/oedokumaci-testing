...
def _get_version(dist: str = "oedokumaci-testing") -> str:
...
def _get_debug_info() -> _Environment:
    ...
    packages = ["oedokumaci-testing"]
    variables = [
        "PYTHONPATH",
        *[var for var in os.environ if var.startswith("OEDOKUMACI_TESTING")],
    ]
    return _Environment(
        intp_name=py_name,
        intp_version=py_version,
        intp_path=sys.executable,
...
def _print_debug_info() -> None:
    ...
    print(f"- __Python__: {info.intp_name} {info.intp_version} ({info.intp_path})")
