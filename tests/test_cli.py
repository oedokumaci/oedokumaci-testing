...
def test_show_help(capsys: pytest.CaptureFixture) -> None:
    ...
    assert "oedokumaci-testing" in captured.out
...
def test_debug_module_main() -> None:
    ...
    result = subprocess.run(  # noqa: S603
        [sys.executable, "-m", "oedokumaci_testing._internal.debug"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_main_module_with_help() -> None:
    ...
    result = subprocess.run(  # noqa: S603
        [sys.executable, "-m", "oedokumaci_testing", "--help"],
        capture_output=True,
        text=True,
        check=False,
    )
    assert result.returncode == 0
    assert "oedokumaci-testing" in result.stdout


def test_main_module_import() -> None:
    """Test that __main__ module can be imported."""
    import oedokumaci_testing.__main__ as main_module  # noqa: PLC0415
